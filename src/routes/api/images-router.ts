...
const checkImage = async (
  ...
  if (existsSync(location) === false) {
    return res
      .status(404)
      .send('<p style="font-size:18; color:red"> Invalid request, image provided does not exist! </p>');
  }
  else if (name && width && height === undefined){
    return res.status(404).send(' <p style="font-size:18; color:red"> A valid image name, width and height must be entered. Try again </p>');
  }
  else if (name === undefined) {
    return res.status(400).send(' <p style="font-size:18; color:red"> Invalid request, Please enter an image name </p>');
